const {
    ...
    modify_Response_Radio,
    modify_Response_Select,
    BulkLineData,
    BulkHeadData,
    modifyForSummary,
    modify_Response_Summary,
    modify_Response_Table,
    addForSummaryTableLine,
    addForSummaryTableLineCSV,
    addForSummaryTableLineCSVTwo,
    processUploadedCSV
  } = require('../../app/backend/models/common_model');
  
  const { sendExternalRequestPost } = require('../../app/backend/custom_requests/external_requests');
  ...
  describe('Your Module Tests', () => {
    ...
    test('modify_Response_Radio should format response data correctly', () => {
      const resp_data = [{ code: 'code1', description: 'desc1' }];
      const selected = 'code1';
      const result = modify_Response_Radio(resp_data, selected);
    ...
    test('modify_Response_Select should format response data correctly', () => {
      const resp_data = [{ code: 'code1' }];
      const selected = 'code1';
      const result = modify_Response_Select(resp_data, selected);
      expect(result).toEqual([{ text: 'CODE1', value: 'code1', selected: true }]);
    });
  
    test('BulkLineData should return line details and table data', () => {
      const data_pack = [{/* data */}];
      const bulk = true;
      const result = BulkLineData(data_pack, bulk);
    ...
    test('BulkHeadData should return correct header data', () => {
      const data_pack = { claimReference: 'ref1', claimReferenceNumber: 'ref2', paymentType: 'typeA', totalAmount: 100, description: 'desc1', frn: 'frn1', currency: 'USD', value: 200, invoiceRequestId: 'id1' };
      const result = BulkHeadData(data_pack, true);
      expect(result).toEqual({
        head: 'Request Id',
        actions: [],
        id: 'id1',
    ...
    test('modify_Response_Summary should format summary data correctly', () => {
      ...
      const result = modify_Response_Summary(items);
      expect(result).toEqual([{
    ...
    test('modify_Response_Table should format table data correctly', () => {
      ...
      const result = modify_Response_Table(items);
      expect(result).toEqual([[
        { text: 'value1' },
        { text: 'value2' }
      ]]);
